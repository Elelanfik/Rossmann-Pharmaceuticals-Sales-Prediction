...
class LSTMModelBuilder:
    ...
    def check_stationarity(self, sample_size=10000):
    
        if len(self.df) > sample_size:
            print(f"Sampling {sample_size} points for stationarity test.")
            sample = self.df['Sales'].sample(n=sample_size, random_state=42)
        else:
            sample = self.df['Sales']
